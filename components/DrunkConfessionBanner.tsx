...
export const DrunkConfessionBanner: React.FC = React.memo(() => {
  ...
  return (
    <div className="fixed top-[calc(4rem+env(safe-area-inset-top))] left-0 right-0 bg-[#FDFBF7] border-b border-[#1A2A3A]/10 z-40 shadow-sm">
      {/* Collapsed View */}
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full px-4 py-3 flex items-center justify-between hover:bg-[#1A2A3A]/5 transition-colors group"
      >
        <div className="flex items-center gap-3">
          <span className="text-base">🍺</span>
          <div className="flex items-center gap-2">
            <span className="text-[#1A2A3A] text-xs font-mono uppercase tracking-widest font-bold">
              Drunk Purchase:
            </span>
            <span className="text-[#FF8A75] text-xs font-mono uppercase tracking-widest font-bold">
              $3k Domain
            </span>
            <span className="hidden sm:inline text-[#1A2A3A]/60 text-xs font-mono">
              · 100% Satire
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono uppercase tracking-widest font-bold text-[#1A2A3A] group-hover:text-[#FF8A75] transition-colors">
            {isExpanded ? '✕ Close' : '💰 Tips'}
          </span>
          <svg 
            className={`w-4 h-4 text-[#1A2A3A] transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
      ...
      >
        <div className="px-4 pb-4 border-t border-[#1A2A3A]/5 bg-[#FDFBF7]">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
              {cryptoAddresses.map((crypto) => (
                <div 
                  key={crypto.label}
                  className="group relative bg-white/80 rounded border border-[#1A2A3A]/5 p-2 hover:border-[#2D9C8E]/30 hover:shadow-lg transition-all duration-200"
                >
