...
export const NavBar: React.FC = () => {
  ...
  return (
    <>
      {/* Mobile Backdrop Bar - Adapts to scroll state and Safe Area Top */}
      <div className={`fixed top-0 left-0 right-0 h-[calc(4rem+env(safe-area-inset-top))] z-40 transition-all duration-500 pointer-events-none ${scrolled ? 'bg-[#FDFBF7]/90 backdrop-blur-xl border-b border-[#1A2A3A]/5 shadow-sm' : ''}`}></div>

      {/* Branding - Fluid Positioning with Safe Area Left */}
      <div 
        className="fixed top-4 left-4 md:top-6 md:left-6 z-50 mix-blend-difference pl-[env(safe-area-inset-left)] pt-[env(safe-area-inset-top)]" 
